...
5. **设置**：常用设置、模型与审阅、运行路径、自动化、连接与通知作为主 Sidebar 内的二级导航。工作台隐藏研究领域、检索词、检索式、运行 profile 和高级底层参数；标题翻译有独立开关，等级复审与 LLM 偏好学习由一个产品开关协调。Local、Zotero Desktop、Zotero Web 三条真实路径互斥，只显示所选路径需要的字段；Zotero Web 凭据与 Desktop 设置分离。普通设置按组原子保存，空替换值表示保持原值；凭据仍逐项明确操作且永不回显。
