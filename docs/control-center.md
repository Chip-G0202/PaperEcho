...
完成弹窗只列出仍有待处理项的常规评级、人工复核或规则建议；数量为 0 的类别不生成文案或跳转按钮。运行路径配置必须可见、可编辑、可保存；Zotero Web 的用户 ID 与 API 地址属于非敏感配置，API Key 只显示配置状态并支持配置、替换和清除，原值不回显。
