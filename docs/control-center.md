...
5. **设置**：常用设置、模型与审阅、自动化、连接与通知作为主 Sidebar 内的二级导航；各组内存在两个以上真实平级任务时，工作区顶部使用三级 tabs。工作台隐藏研究领域、检索词、检索式和高级底层参数；检索来源使用复选框，保留检索天数。标题翻译与偏好学习分别成组保存；`llm_review.batch_size` 仅控制 Stage 1 等级复审的单次 LLM 请求分批，属于内部性能参数，不在普通设置中展示。凭据因安全边界仍逐项明确操作，原配置 owner、校验和安全写入契约不变。
