...
App Shell 由固定 Sidebar、页头和主内容组成；L1 是左侧主导航，L2 是对应主项下的嵌套导航，L3 是工作区顶部的平级任务 tabs，筛选器不充当导航。hash 保存当前导航状态并兼容旧反馈链接。窄窗口通过“导航菜单”展开，正文随导航向下排列；宽屏可为真正有用的摘要、进度和快捷键提供 context rail，不为填充空间制造内容。当前入口为概览、文献、反馈、设置、系统，不放置尚未实现的未来模块。Web header、favicon、Windows ICO 与 macOS ICNS 均由 `workflow/tools/web/static/paperecho-mark.svg` 这一 canonical mark 派生。

完成弹窗只列出仍有待处理项的常规评级、人工复核或规则建议；数量为 0 的类别不生成文案或跳转按钮。运行路径配置必须可见、可编辑、可保存；Zotero Web 的个人文库数字 User ID 属于非敏感配置，API Key 只显示配置状态并支持配置、替换和清除，原值不回显；PaperEcho 项目根目录单列为本地工作区，不冒充 Zotero 官方配置。

正式 Control Center 不提供示例模式、演示入口或演示数据；开发验证数据仅保留在测试 fixture 中，不进入交付页面。“全部人工处理已完成”仅在常规评级、人工复核与规则建议三类待办均清零后显示。

Settings 按分类显示字段；模型与 API 字段分开呈现当前状态和空替换输入，空值不会清除配置。功能关闭时隐藏详细模型字段，重新开启后原值仍由配置 owner 保留。Sidebar 宽度随桌面视口调整，正文与 context rail 共同使用剩余空间。凭据仍为空密码框，仅写入、不回显。研究评价草稿只在当前页面会话内保留，不写浏览器持久存储。System 只展示已有状态的安全摘要，不显示绝对路径、不把历史证据当作实时健康检查。没有结构化推荐依据时不生成“Why Recommended”。
...
- 外部 Agent 每天北京时间 15:00 唤醒 PaperEcho；PaperEcho 自己根据最近成功 Weekly 的计划时隙选择 Radar 或 Weekly。漏跑的历史 Radar 不补跑，Control Center 无需保持打开。概览的“今日任务”只读复用共享调度决策；实现机制留在文档，日常 UI 只显示任务状态与必要操作。当前 v2.4 不注册 OS Scheduler；Local 路径不支持 scheduled-daily。
